...
| Background commands started by subagents no longer stop at one hour (was `CLAUDE_SUBAGENT_BG_SHELL_MAX_MS`) | 2.1.257 | `hooks/subagent-no-background.sh` — an orphaned loop is now unbounded, so subagents get no `run_in_background` at all |
