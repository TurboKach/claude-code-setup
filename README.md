...
- **2026-09-01 references + version stamp (from a v2.1.238→v2.1.252 changelog read):** the doctrine's sources lived only in one machine's session memory, so nothing shipped with the kit and drift was invisible — the last changelog read was v2.1.237 while the machine ran v2.1.252. `docs/references.md` now holds them with a "validated against" stamp, and an optimize request starts by diffing the changelog from that stamp. Three rules relaxed by what the diff found: `CLAUDE_CODE_SUBAGENT_MODEL` became a *default* rather than an override in 2.1.251 (agent `model:` and per-spawn pins win), so the kit sets it to `sonnet` as a floor and the per-spawn pin stops being a guard against tier inheritance; a `maxTurns` stop returns partial output continuable via `SendMessage` since 2.1.246, so it is no longer a respawn case; and the teammate-mailbox rationale ("a report arrives only if the agent remembers to send it") was fixed in 2.1.251 — teammates' final answers now arrive in the idle notification and `ListAgents` lists them, so unnamed spawns stay the default on delivery-simplicity grounds, not breakage.
