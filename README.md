...
model both take precedence over it. The pinned roles keep their frontmatter
...
`env` changes are read at session start — restart Claude Code
...
- **2026-09-04 subagent orphan shells (from the clipsy_ios side-handle word-floor arc):** the master reported "all done" while a shell the `fixer` had started 54 minutes earlier was still alive — `until [ -f <task>.output.done ]; do sleep 5; done` as a `run_in_background` Bash. The chain: `xcodebuild test` with no `timeout` → auto-backgrounded at the 2-minute default → `sleep 90; tail` blocked by the harness with "use Monitor with an until-loop" → the kit's `tools:` lists strip Monitor and TaskStop → a hand-rolled background poll on a marker the harness never writes → with fork mode on (default since 2.1.232) the fixer is a background subagent, whose background commands keep running past its final response (docs), uncapped since 2.1.260. Measured over 8 sessions since 2026-08-25: 25 subagents auto-backgrounded, ~30 of ~40 of those calls passed no `timeout` despite the agent-definition line, 16 hit the sleep block, 2 orphaned pollers, 5 foreground polls burned their full timeout — prose does not hold. Three levers, all mechanical: `BASH_DEFAULT_TIMEOUT_MS=900000` in the merged settings, `hooks/subagent-no-background.sh` (PreToolUse, rules above), and the corrected doctrine — `feature-workflow` had claimed a subagent's backgrounded call ends at its final response, which the docs say only for foreground subagents. Rejected: `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` (would kill the master's backgrounded codex run) and re-adding Monitor to executors (a Monitor script on the same nonexistent marker orphans the same way). The clipsy master's own diagnosis blamed the harness and the wrong agent; the transcripts, not the report, settled it.
- **2026-09-04 first four arcs on the updated kit measured (clipsy_ios in-place text edit, bali traffic-sources / SEO / RU stats; 34 `codex-challenge.sh` runs, ~105 subagents):** pins, single-writer, per-step rounds beside the next executor, turn-cap WIP + respawn, 529 park-and-notify, push gating all held; peak master context 249k–461k. Four gaps closed here: two unattended arcs waited ~9 h each at `ExitPlanMode` after the owner had delegated approval (docs: nothing auto-approves the plan prompt; only the owner leaves plan mode) → delegated-approval branches in the AFK gate; the traffic arc shipped 403ing beacons through seven clean codex rounds because its verification step used curl → planner writes and plan-reviewer blocks on a positive-path real-client check; a `/tmp` worktree silently skipped environment-gated tests and spawned a fixer on a phantom regression → test evidence carries executed/skipped/failed counts; a bare `/tmp/codex-step1.md` `--out` while a second arc ran in the same repo → scratchpad paths in both call strings. Re-stamped to 2.1.260 (only kit-relevant entry: subagent background commands no longer stop at one hour).
- **2026-09-04 `codex-challenge.sh` stops leaking DerivedData (from the clipsy_ios in-place-text-edit arc):** five pinned reviews on 2026-09-03 left 18.6 GiB under `~/Library/Developer/Xcode/DerivedData` and took the disk to 1.7 GiB free. The read-only sandbox does deny those writes; the escape was an `allow` prefix rule for `xcodebuild` in `~/.codex/rules` (smart approvals add them), which under the inherited `on-request` policy runs the command outside the sandbox. The script now passes `-c 'approval_policy="never"'` (Codex's documented policy for non-interactive runs; verified to keep `xcodebuild` sandboxed) and its EXIT trap and dead-run sweep also delete any DerivedData keyed to the review checkout. Claude Code hooks were checked and rejected: `WorktreeRemove` only fires for Claude-managed worktrees and `SessionEnd` has a 1.5 s budget, so cleanup lives in the script.
