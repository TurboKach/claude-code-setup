...
| `agents/fixer.md` | Fixes one review round's finding set on the session's own branch, red-then-green — a bounded task at a known `file:line`, so it runs cheaper than a plan step *(Sonnet medium; Opus only for a same-mechanism structural fix)* |
| `agents/codex-runner.md` | Runs one `/codex challenge` round on a diff range and returns the ≤2k triaged verdict; frontmatter hooks deny a backgrounded Bash and block a stop while `codex exec` is alive, because a finished subagent is never re-woken by its own background task *(Sonnet medium)* |
...
| `hooks/codex-runner-hooks.sh` | Frontmatter hooks for `agents/codex-runner.md`: denies a backgrounded Bash call and blocks a stop while `codex exec` is alive |
