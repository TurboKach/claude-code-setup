...
| `agents/codex-triage.md` | Reads one finished `/codex challenge` output file, checks each finding against the pinned checkout, and returns the ≤2k triaged verdict; the run itself is a background Bash in the master, the only context the harness re-wakes on completion *(Sonnet medium)* |
