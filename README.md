...
- **2026-08-18 two paths + ledger:** one-shot (≤3 files, no design/UI choice, reversible — no plan ceremony, but `/codex challenge` still runs on the diff) vs pipeline, with the call stated in one line; every spawn pins `model:` (never `fable`; Explore → sonnet, since built-in Explore inherits the session model); the planner ends each plan with a Done-when checklist the master ticks in the plan doc (no Stop hook — deliberately prose-only for now).
