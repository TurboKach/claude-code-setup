...
| `skills/feature-workflow/scripts/codex-challenge.sh` | Range-scoped adversarial `codex exec` on exactly `<base>..<head>`; optional self-removing pinned worktree; `gtimeout 900`, 3 attempts / 5 min (a 400 from a rejected model stops retrying immediately); model + effort from `CODEX_REVIEW_MODEL` / `CODEX_REVIEW_EFFORT` (default `gpt-5.6-sol` / `medium`), recorded in the verdict header — these keys arrive via the Claude Code session's environment, so a run started from a bare terminal outside a session falls back to the default; writes the verdict file, prints its path on stdout and the elapsed wall-clock time on stderr |
