...
- **Pushes to remote are user-approved, always.** `git push` — any remote, any branch — only after my explicit approval in the session (me firing `/ship` or `/land-and-deploy` counts). Staying unpushed has a cost worth naming when you report an arc complete: ungated commits accumulate, and one arc's gate ends up re-reviewing several arcs' work. In the clipsy session that reached 47 unpushed commits, the whole-range gate was 486KB across 56 files and had to be split seven ways. Surface that as a reason to push now — never as a reason to push without asking, and never as an obligation to ask when I've said the work stays local.
- **`/codex` ship gate.** Never ship (push, PR, `/ship`) a pipeline's work until gstack's `/codex` skill has run in **challenge** mode once on the feature's whole diff — `Skill(codex, "challenge <feature-base-sha>..HEAD")`, launched by the master as one background Bash on a pinned worktree and triaged by the kit's `codex-triage` agent — and its triaged verdict is shown. Internal reviewer subagents do NOT satisfy this gate. If `/codex` hasn't run, say "unreviewed, not merging".
...
- **Gating is right; idling while gated is not.** Before opening a gate, start whatever work genuinely doesn't depend on its answer, so my thinking time isn't also your wall clock. When there is no such work — which is normal at scope and architecture gates — end the turn and wait; never manufacture speculative work, and never start anything the answer could invalidate. Separately, ending a turn with no question pending is right when the work is done and wrong when it isn't: if there's more to do and nothing actually needs me, keep going rather than reporting progress and waiting. In one six-hour session a 42-minute gate and a 19-minute question-free turn end both ran with zero agents in flight.
...
- **Delegates spawn unnamed.** Passing `name:` turns a subagent into a mailbox teammate whose report reaches the master only if it remembers to SendMessage; unnamed spawns auto-deliver their final report (inline when foreground, task-notification when background). Names are only for parallel teams that need live coordination. **Every spawn pins `model:`** — an unpinned subagent silently inherits whatever tier the master is running: `sonnet` for executors, fixers, `codex-triage` and `general-purpose`; codebase search → the kit's `explorer` agent (sonnet, medium, read-only) instead of the built-in `Explore`, which inherits the session's model and effort; `opus` for planner / plan-reviewer / team-reviewer, for an executor only when the plan marks that step Opus with a reason, and for a fixer only in the same-mechanism structural case with the reason stated at spawn; **`fable` never in a subagent.** And silence is not progress: past an agent's expected window — or when its result is an API error — read its transcript under the session's `subagents/` dir, salvage what finished, respawn a fresh agent for only the remainder — up to 2 respawns for an API-error death, then park with a push notification; if my own harness retry was cancelled (Esc) or exhausted, that path stays manual and you say so. But measure the window from the agent's own spawn timestamp and last transcript write, never from how long *you* have been waiting or a goal check-in's "deferred N min" — a runner spawned 20 s ago is not stalled; killing a healthy agent costs a full re-run.
