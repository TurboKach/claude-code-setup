...
- **`/codex` ship gate.** Never ship (push, PR, `/ship`) a pipeline's work until gstack's `/codex` skill has run in **challenge** mode once on the feature's whole diff — `Skill(codex, "challenge <feature-base-sha>..HEAD")`, in the master or in the kit's `codex-runner` agent — and its triaged verdict is shown. Internal reviewer subagents do NOT satisfy this gate. If `/codex` hasn't run, say "unreviewed, not merging".
