...
3. **Review + refine** → `/autoplan`. Runs CEO + Design + Eng + DX review skills sequentially, auto-decides mechanical questions via 6 principles, surfaces only taste decisions at a final approval gate. Updates the plan in place — but "in place" does NOT exempt the master from the no-hand-authoring rule: each review phase's plan-integration edits are applied by a subagent fed that phase's findings; the master runs only the gates. The master also never Reads the review sub-skill files when the voices run as subagents — pass the skill path in the subagent prompt instead (2026-07-27: the master Read ~288k chars of sub-skills the subagents then loaded again themselves, and hand-integrated review findings via 30 inline plan edits).
...
- Keeping the master thin via delegation is what lets one session run e2e. If it nevertheless approaches the context ceiling (~500k tokens; 600k absolute max), do a **deliberate, user-assisted handoff** to a fresh master session — don't silently push past it. `/context-save` + `/context-restore` are the handoff bridge.
- **Master budget: a normal feature arc finishes under ~400k with no compression machinery.** Crossing ~400k mid-goal is a *defect signal*, not a reason to compress or hand off: stop, post the cost checkpoint, and name what's flooding the context (teammate chatter, full transcripts, re-injections) so the flow gets fixed — the 2026-07-28 arc hit 736k on a normal-sized task purely from transport noise, not task size. During unattended `/goal` runs, post a one-line cost checkpoint at every unit boundary (elapsed time, review rounds used, approximate context size) so overruns surface mid-arc instead of in a post-mortem, and END the session the moment the goal completes — never leave a finished session idling overnight (each cache expiry re-writes the full context at premium pricing; one finished ~730k-context session re-paid it twice).
...
- **Kill delegated agents the moment their unit closes.** When a background agent/teammate has reported and its output is ingested (commit merged, verdict triaged, report received), stop it in the same turn — agents are per-unit disposables, never kept warm for possible reuse. Before declaring a multi-agent goal complete, enumerate live agents and confirm zero remain; I must never have to kill leftover idle agents by hand (it happened in two consecutive sessions on 2026-07-28: 16 leftovers in one, 2 in the other).
- **Serial pipelines never run as named teammates.** A build→review→fix loop run as 13 SendMessage teammates generated 140 inbound master wakes — 98 of them pure idle_notifications, each a full ~430k-context API call (42M context tokens for zero information). Fire-and-return background subagents produce one summary each; a teammate that is somehow unavoidable gets stopped the moment its unit closes.
...
The measured burn model (2026-07-28 forensics on a 21-hour, 7-unit arc that consumed ~40% of a weekly quota): **every agent turn re-pays its entire context as cache-read, so cost ≈ turn-count × context size and grows quadratically in a long-lived agent.** A 641-turn executor averaging ~400k context cost ~249M cache-read tokens; build-log piping (the only mechanically-phrased rule, followed 115/116 times) addressed a cost three orders of magnitude smaller. Agent *lifetime* is the lever, not result size or output verbosity.

- **Retirement is mechanical, not prose.** Executor spawn prompts carry the budget inline: "if you exceed ~200k context or ~250 turns — commit WIP, write a handoff file to the scratchpad, and stop." The orchestrator kills+respawns at the threshold. A ceiling that lives only in a memory file is invisible to the subagent it governs — in the measured arc, all 13 heavy agents blew past 200k with zero handoff attempts (worst: 705k); retiring at 200k alone would have saved ~55% of the total burn.
- **Fresh agent per review round.** One codex/review round = one fresh wrapper agent — a round needs only the diff range and prompt (small files), never the accumulated context of prior rounds. Post-review fixes go to a fresh fixer agent fed the plan section + unit diff + verdict, never back to the original executor carrying its full implementation history at peak context.
- **Verdict size contract.** Reviewer/codex agents return a structured verdict ≤2,000 chars: counts, one-line findings with file:line, real/regression/test-gap/theoretical tags. Full transcripts stay on disk and the master NEVER Reads challenge-output files (one master Read 36 full transcripts = ~108k tokens of permanent context, re-paid by every later call).
